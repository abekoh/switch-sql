...
def is_begin_bracket(input):
    return "(" in input and ")" not in input


def is_end_bracket(input):
    return ")" in input and "(" not in input
...
def expand(parsed, num):
    ...
    for i, current in enumerate(parsed):
        if is_begin_bracket(current):
            depth += 1
        if is_end_bracket(current):
            depth -= 1
        if depth == 1 and is_begin_bracket(before):
            start_i = i
        if depth == 0 and is_end_bracket(current):
            end_i = i - 1
        before = current
